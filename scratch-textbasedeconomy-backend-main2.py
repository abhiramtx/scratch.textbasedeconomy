...
try:
  def main():
    ...
    data['Air_Heads2']['items']['ecoins']['amount'] = float('inf')
    data['Air_Heads2']['job']['jobNum'] = '-1'
    
    shop = jsonpickle.decode(db['shop'])
    ...
    def shoplist():
      ...
      for i in itemNames:
        return_data.append(f"| {i}:")
        return_data.append(f"| Stock: {shop[i]['stock']} items")
        return_data.append(f"| Buy price: {shop[i]['buyPrice']} ecoins")
        return_data.append(f"| Sell price: {shop[i]['sellPrice']} ecoins")
        return_data.append(f"| Usable: {shop[i]['useable']}")
        if shop[i]["useable"] == "True":
          return_data.append(f"| Item use time: {shop[i]['expireTime']} seconds")
        return_data.append(f"| Description: {shop[i]['description']}")
        return_data.append("| ----------")
      return_data.append(f"| Shop prices will update in: {3600 - (round(time.time()) - fullData['gameData']['lastShopRefresh'])} seconds")
      return_data.append(f"| Shop stocks will update in: {1800 - (round(time.time()) - fullData['gameData']['lastStockRefresh'])} seconds")
      return return_data
    ...
    def tick():
      while True:
        ...
        #RESET SHOP BEFORE RERUNNING
        #shopKeys = list(shop.keys())
        #for i in shopKeys:
          #shop[i]['buyPrice'] = shop[i]['originalPrice']
          #shop[i]['sellPrice'] = shop[i]['originalSell']
          #shop[i]['demand']['hour1'] = 0
          #shop[i]['demand']['hour2'] = 0
          #shop[i]['stock'] = 1000
        #print('done')
        ...
        for i in users:
          ...
          if not days < 1:
            if (data[i]["bank"]["bankCoins"] * 0.1) * days < 50000:
              ...
              if ecoins != 0:
                inbox = data[i]["inbox"]
                inbox.append(f"Bank: Could not supply interest, cannot supply {(data[i]['bank']['bankCoins'] * 0.1) * days} ecoins.")
                inbox.append(time.time())
                inbox.append(86400)
                data[i]["inbox"] = inbox
        ...
        #shop
        if round(time.time()) - fullData["gameData"]["lastShopRefresh"] > 3600:
          shopKeys = list(shop.keys())
          for i in shopKeys:
            if shop[i]['buyPrice'] != 'not buyable':
              shop[i]['buyPrice'] += ((shop[i]['demand']['hour2']-shop[i]['demand']['hour1'])//100)*shop[i]['originalPrice']
            if shop[i]['sellPrice'] != 'not sellable':
              shop[i]['sellPrice'] += ((shop[i]['demand']['hour2']-shop[i]['demand']['hour1'])//100)*shop[i]['originalSell']

            print(shop[i]['demand']['hour1'])
            print(shop[i]['demand']['hour2'])
            
            shop[i]['demand']['hour1'] = shop[i]['demand']['hour2']
            shop[i]['demand']['hour2'] = 0
         
          fullData["gameData"]["lastShopRefresh"] = time.time()

        if round(time.time()) - fullData["gameData"]["lastStockRefresh"] > 1800:
          shopKeys = list(shop.keys())
          for i in shopKeys:
            if shop[i]['demand']['hour2']-shop[i]['demand']['hour1'] != 0:
              if shop[i]['demand']['hour2']-shop[i]['demand']['hour1'] < 0:
                shop[i]['stock'] += 6520
              else:
                shop[i]['stock'] += 12500
          fullData["gameData"]["lastStockRefresh"] = time.time()
